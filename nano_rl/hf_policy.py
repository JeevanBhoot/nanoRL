...
class HFPolicy:
    ...
    def generate_with_logprobs(self, prompts: Sequence[str], gen_cfg: Optional[GenConfig] = None, k: int = 1):
        """Sample `k` continuations per prompt, score them, and decode."""
        if k < 1:
            raise ValueError("k must be >= 1 for _sample_and_score.")
        gen_cfg = gen_cfg or GenConfig()

        # 1) Sample actions (no gradients)
        inputs, lens = self._tokenize(prompts)
        with torch.no_grad():
            out = self.model.generate(
                **inputs,
        ...
        logp = torch.log_softmax(logits[:, :-1, :], dim=-1)                               # [B*k, T_max-1, V]
        tgt = seqs[:, 1:]                                                                 # [B*k, T_max-1]

        # Mask out the prompt and padding tokens
        _, Tm1 = tgt.shape
        idx = torch.arange(Tm1, device=self.device).unsqueeze(0)                          # [1, T-1]
        prompt_lens = (lens.repeat_interleave(k) if k > 1 else lens).unsqueeze(1)         # [B*k, 1]
        seq_lens = attn.sum(dim=1, keepdim=True)                                          # [B*k, 1]
        ...
        logprob_sums = tok_logp.sum(dim=1)                                                # [B*k]

        # Decode generated continuations
        prompt_lens_flat = prompt_lens.squeeze(1)
        seq_lens_flat = seq_lens.squeeze(1)
        texts: List[str] = []
        for r in range(seqs.size(0)):
            start = int(prompt_lens_flat[r].item())
            end = int(seq_lens_flat[r].item())
            gen_tokens = seqs[r, start:end]
            texts.append(self.tokenizer.decode(gen_tokens, skip_special_tokens=True))

        if k > 1:
            texts = [texts[i*k:(i+1)*k] for i in range(len(prompts))]
            logprob_sums = logprob_sums.view(len(prompts), k)
